...
_cached_time: Optional[datetime.datetime] = None
...
class SecurityBlockException(Exception):
    ...
    def __init__(self, message: str, block_type: str):
        ...
        self.block_type = block_type  # "BANNED", "MALICIOUS", "OFF_TOPIC", "BUDGET_EXCEEDED"
...
def get_cached_security_config(repo: FirestoreRepository) -> Dict[str, Any]:
    ...
    if _cached_config is None or _cached_time is None or (now - _cached_time).total_seconds() > 300:
...
async def before_agent_callback(callback_context: CallbackContext) -> None:
    ...
    locale_suffix = "de" if "de" in locale.lower() else "pt" if "pt" in locale.lower() else "en"
    ...
    if hashed_ip in _local_banned_cache:
        if now < _local_banned_cache[hashed_ip]:
            logger.warning(f"Fast-blocking banned client signature {hashed_ip} from local active cache.")
            banned_msg = config.get("responses", {}).get(f"banned_{locale_suffix}", "Access blocked.")
    ...
    if repo.is_signature_banned(hashed_ip):
        logger.warning(f"Fast-blocking banned client signature {hashed_ip} from Firestore database.")
    ...
    if callback_context.user_content and callback_context.user_content.parts:
        prompt = "".join([part.text for part in callback_context.user_content.parts if part.text]).strip()
    ...
    for kw in blocklist_keywords:
        if kw.lower() in prompt.lower():
            ...
            await _handle_safety_violation(callback_context, repo, config, hashed_ip, prompt, "KeywordMatch", locale_suffix)

    # Regex matching
    for pattern in injection_regexes:
        if re.search(pattern, prompt, re.IGNORECASE):
            logger.error(f"Safety violation triggered by injection regex match: '{pattern}'")
            await _handle_safety_violation(callback_context, repo, config, hashed_ip, prompt, "RegexMatch", locale_suffix)
    ...
    try:
        ...
        response = client.models.generate_content(
            ...
            contents=[
                genai_types.Content(role="user", parts=[
                    genai_types.Part.from_text(text=f"{classifier_prompt}\n\nUser input to review: \"{prompt}\"")
                ])
            ],
        ...
        if "MALICIOUS" in decision:
            logger.error("Safety violation triggered by semantic classifier evaluation.")
            await _handle_safety_violation(callback_context, repo, config, hashed_ip, prompt, "ClassifierBlock", locale_suffix)
        ...
        logger.error(f"Error during semantic safety classification: {e}. Defaulting to safe passage.")
...
async def _handle_safety_violation(
    ...
    logger.warning(f"Client {hashed_ip} has committed {violations_count} safety violations in the past hour.")

    if violations_count >= 3:
        # The Gavel: Auto-ban signature for 24 hours
        logger.error(f"🤠 The Sheriff Guard has banned client {hashed_ip} due to 3 active violations!")
        repo.ban_signature(hashed_ip, duration_hours=24)
        _local_banned_cache[hashed_ip] = datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(hours=24)
        banned_msg = config.get("responses", {}).get(f"banned_{locale_suffix}", "Access blocked.")
        raise SecurityBlockException(banned_msg, "BANNED")
...
async def after_agent_callback(callback_context: CallbackContext) -> Optional[genai_types.Content]:
    ...
    if total_tokens > 0:
        anon_id = get_anonymous_id()
        logger.info(f"Aggregated invocation {invocation_id} used {total_tokens} tokens. Logging to budget.")
        repo = FirestoreRepository()
